...
> **⚠️ Retrieval ground-truth needs re-labeling for ChromaDB.**
> This dataset was authored against the old pgvector store, whose chunk IDs were
> `icar:crop:{crop}`. This branch uses the **ChromaDB** vector MCP (`chroma_server`,
> dual collections `icar_knowledge_en` / `icar_knowledge`), which has **different
> chunk IDs**. Until `relevant_chunk_ids` are re-labeled against the live Chroma
> store, retrieval metrics (recall/precision/MRR/hit) read ~0 — *grounding, judge,
> keyword and behavior metrics are unaffected.* Discover the real IDs with:
> ```bash
> curl -s localhost:9103/mcp -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"query_knowledge_base","arguments":{"query":"rice rainfall","top_k":5}}}' | python3 -m json.tool
> ```
> then replace each case's `relevant_chunk_ids`.
...
curl -s localhost:9103/mcp -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"list_collections","arguments":{}}}'
```
